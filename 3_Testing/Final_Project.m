...
for idx=baseFileName;
    ...
    if size(mserStats, 1) > 0;
        ...
        h_Text=uicontrol('Style','text','String','Recognized Car Plate Number:',...
            'Position',[430 210 150 20],'FontSize',10);
        ...
        for i=1:sizeofletters
            ret_letter=CheckLetter2(cropped_letters(:,((i-1)*18+1):((i-1)*18+18)));
            if ret_letter == 0
                RecognizedText(i)='-';
            else
                RecognizedText(i)=ret_letter;
            end
        end
        
        
        set(h_Edit,'String',RecognizedText);
